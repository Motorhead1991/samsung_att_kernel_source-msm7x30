...
static void set_pmic_vibrator(int on)
{
	...
	//printk(KERN_INFO, "[VIB] %s, %s (%d, %d, %d)(%d, %d)\n",__func__,on ? "ON":"OFF", vreg_vib->id, vreg_vib->status, vreg_vib->refcnt, is_vibe_on, sec_vibrater_ctrl);
	printk(KERN_INFO  "[VIB] %s, %s %d,%d,%d,%d\n",__func__,on ? "ON":"OFF",
				vreg_vib->id,vreg_vib->status, vreg_vib->refcnt, is_vibe_on);
	
}
#else
static void set_pmic_vibrator(int on)
{
#if defined(CONFIG_MACH_ANCORA) || defined(CONFIG_MACH_GODART) || defined(CONFIG_MACH_ANCORA_TMO)
	int rc; 
#endif
