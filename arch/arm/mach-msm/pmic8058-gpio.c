...
extern int Pmic_gpio_sens_irq;
extern int Pmic_gpio_wakeup_rescan;

void msm_pmic_gpio_enter_sleep(void);
void msm_pmic_gpio_exit_sleep(void);
#endif
...
#ifdef CONFIG_MACH_APACHE
#define PM8058_GPIO(pm_gpio)  (pm_gpio - 1)


static int pm8058_gpio_kp_config_drv(int gpio_start, int num_gpios)
{
	int	rc;
...
static int pm8058_gpio_kp_config_sns(int gpio_start, int num_gpios)
{
	int	rc;
...
static int pmic8058_gpio_kp_enable_disable(int enabe)
{
	int rc;
	u8 ctrl_reg = 0;
	...
	if(enabe)
	{
		ctrl_reg |= KEYP_CTRL_KEYP_EN;
	}
	else
	{
		ctrl_reg &= ~KEYP_CTRL_KEYP_EN;
	}
	rc = pm8058_write(pm8058_gpio_chip.pm_chip, KEYP_CTRL, &ctrl_reg, 1);
	if (rc < 0)
		pr_err("%s: Failed on pm8058_write(): rc=%d (GPIO config)\n",
				__func__, rc);		
	mutex_unlock(&pm8058_gpio_chip.pm_lock);
...
void msm_pmic_gpio_enter_sleep(void)
{
	...
	struct pm8058_gpio kypd_drv_kb = {
		.direction		= PM_GPIO_DIR_OUT,
		.pull			= PM_GPIO_PULL_NO,
	...
	disable_irq(Pmic_gpio_sens_irq);
	
	if(on_call_flag || on_fmradio_flag) 
	{
		rc = pm8058_gpio_config(PM8058_GPIO(11), &kypd_drv_kb); //kbc2
		if (rc) {
		pr_err("%s: FAIL pm8058_gpio_config 11 (): rc=%d.\n",
			__func__, rc);
		return rc;
		}			
	} 
	else 
	{
		...
		rc = pm8058_gpio_config(PM8058_GPIO(2), &kypd_drv_kb); //kbr1 / pmic_gpio 2
		if (rc) {
		pr_err("%s: FAIL pm8058_gpio_config 2 ()  : rc=%d.\n",
			__func__, rc);
		return rc;
		}
...
void msm_pmic_gpio_exit_sleep(void)
{
	printk("msm_pmic_gpio_exit_sleep + \n");
	pmic8058_gpio_kp_enable_disable(0);
	pm8058_gpio_kp_config_sns(PM8058_GPIO(1),2);
	pm8058_gpio_kp_config_drv(PM8058_GPIO(9),3);
	pmic8058_gpio_kp_enable_disable(1);
	enable_irq(Pmic_gpio_sens_irq);
	Pmic_gpio_wakeup_rescan = 1;
	printk("msm_pmic_gpio_exit_sleep - \n");
