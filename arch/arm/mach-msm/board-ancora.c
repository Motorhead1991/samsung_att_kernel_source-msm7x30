...
#ifdef CONFIG_AUTO_INPUT_EVENT_GENERATOR
...
#ifdef CONFIG_AUTO_INPUT_EVENT_GENERATOR
/* key handler */
extern void pmic8058_auto_event_handler(unsigned int keyevt);

/* touchkey handler */
#ifdef CONFIG_TOUCHSCREEN_MELFAS_MCS8000
extern void mcs8000_touch_run(int pos_x, int pos_y, int pos_str);
extern void mcs8000_touchkey_run(unsigned int keyevt);
#else
void mcs8000_touch_run(int pos_x, int pos_y, int pos_str)
{
}
void mcs8000_touchkey_run(unsigned int keyevt)
{
}
#endif

void ancora_key_event_handler(unsigned int keyevt)
{
    /* call key event function */
    pmic8058_auto_event_handler(keyevt);
}

void ancora_touch_event_handler(int pos_x, int pos_y, int pos_str)
{
    /* call touch event funtion */
    mcs8000_touch_run(pos_x, pos_y, pos_str);
}

void ancora_touchkey_event_handler(unsigned int keyevt)
{
    /* call touchkey event function */
    mcs8000_touchkey_run(keyevt);
}

static struct auto_input_event_generator_platform_data ancora_auto_input_generator_data = {
    .event = AUTO_EVENT_ALL,//AUTO_EVENT_KEY | AUTO_EVENT_TOUCHKEY | AUTO_EVENT_TOUCH,

    .repeat_time_key        = 30000,        // every 30 second
    .repeat_time_touchkey   = 5000,         // every 5 second
    .repeat_time_touch      = 300,          // every 0.3 second

    .auto_start = 1,
    .auto_start_time = 120000,  //2 min

    .key_code = {KEY_HOME, KEY_VOLUMEDOWN, KEY_UP},
    .touchkey_code = {KEY_BACK, KEY_MENU, KEY_SEARCH},

    .touch_matrix_x = 480,
    .touch_matrix_y = 800,
    .touch_matrix_str = 110,

    .key_event_handler       = ancora_key_event_handler,
    .touchkey_event_handler  = ancora_touchkey_event_handler,
    .touch_event_handler     = ancora_touch_event_handler,

};

static struct platform_device ancora_auto_input_generator = {
    .name   = "auto_input_event_generator",
    .id     = -1,
    .dev    = {
        .platform_data = &ancora_auto_input_generator_data,
    },
};
#endif
