...
/* not use ANCORA_EXT_SD_ESD_CHECK_ROUTINE */
	...
	do {
		...
		if (brq.cmd.error || brq.data.error || brq.stop.error) {
			if (brq.data.blocks > 1 && rq_data_dir(req) == READ) {
				...
				printk(KERN_WARNING "%s: error type, cmd error(%d) "
				       "data error(%d) stop error(%d)\n", req->rq_disk->disk_name,brq.cmd.error,brq.data.error,brq.stop.error );
				BUG_ON(	!strcmp(req->rq_disk->disk_name, "mmcblk0"));
#endif
				continue;
			}
