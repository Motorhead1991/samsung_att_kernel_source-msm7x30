...
int usb_ept_queue_xfer(struct msm_endpoint *ept, struct usb_request *_req)
{
	...
	if (!atomic_read(&ui->configured) && (ept->num != 0)) {
		...
		spin_unlock_irqrestore(&ui->lock, flags);
		if(!power_down)
			dev_err(&ui->pdev->dev, "usb_ept_queue_xfer() called while offline\n");
		return -ESHUTDOWN;
	}
...
static int msm72k_pullup_internal(struct usb_gadget *_gadget, int is_active)
{
	...
	//problem - lose MODEM port after reboot during automation test
	//add - wait for some time for pullup to be disabled.
		mdelay(20); 
	#endif
	}
