...
int Pmic_gpio_sens_irq = 0;
int Pmic_gpio_wakeup_rescan = 0;
#endif
...
struct pmic8058_kp *g_kp;
void pmic8058_auto_event_handler(unsigned int keyevt)
{
    input_report_key(g_kp->input, keyevt, 1);
    mdelay(1);
    input_report_key(g_kp->input, keyevt, 0);
}
#endif
...
static int __devinit pmic8058_kp_probe(struct platform_device *pdev)
{
	...
	Pmic_gpio_sens_irq = kp->key_sense_irq;
#endif
...
static int pmic8058_kp_suspend(struct device *dev)
{
	...
	Pmic_gpio_wakeup_rescan = 0;
	
	if (device_may_wakeup(dev) && !pmic8058_kp_disabled(kp)) {

		printk("pmic8058_kp_suspend Pmic_gpio_wakeup_rescan = %d \n",Pmic_gpio_wakeup_rescan);
		...
		rc = pm8058_gpio_config(PM8058_GPIO(9), &kypd_drv_kb); //kbc0
			if (rc) {
		pr_err("%s: FAIL pm8058_gpio_config 9 (): rc=%d.\n",
				__func__, rc);
			return rc;
			}			
...
static int pmic8058_kp_resume(struct device *dev)
{
	...
	if (device_may_wakeup(dev) && !pmic8058_kp_disabled(kp)) {
		...
		printk("pmic8058_kp_resume  Pmic_gpio_wakeup_rescan = %d \n",Pmic_gpio_wakeup_rescan);

		if(!Pmic_gpio_wakeup_rescan)
	{
		kp->ctrl_reg &= ~KEYP_CTRL_KEYP_EN;
		pmic8058_kp_write_u8(kp, kp->ctrl_reg, KEYP_CTRL);
		pm8058_kp_config_sns(PM8058_GPIO(1),2);
		pm8058_kp_config_drv(PM8058_GPIO(9),3);
		kp->ctrl_reg |= KEYP_CTRL_KEYP_EN;
		pmic8058_kp_write_u8(kp, kp->ctrl_reg, KEYP_CTRL);		
		enable_irq(kp->key_sense_irq);
	}
