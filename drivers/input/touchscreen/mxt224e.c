...
uint8_t calibrate_chip(void)
{
    ...
    printk(KERN_ERR"[TSP] calibrate_chip : ENTER \n");
...
static int __devinit mxt224_init_touch_driver(struct mxt224_data *data)
{
	...
	ret = read_mem(data, OBJECT_TABLE_START_ADDRESS,
			...
			(u8 *)object_table);
	if (ret){
		printk(KERN_ERR "[TSP] %s,error line %d, ret=%d ",__func__, __LINE__, ret);
		goto err;
	}
...
{
	ERR_RTN_CONTIOIN rtn = ERR_RTN_CONDITION_IDLE;
...
static void mxt224_late_resume(struct early_suspend *h)
{
	...
	struct mxt224_data *data = container_of(h, struct mxt224_data,
								early_suspend);
    printk(KERN_ERR "[TSP] mxt224_late_resume : ENTER \n");
	...
	calibrate_chip();	
	printk(KERN_ERR "[TSP] mxt224_late_resume : END \n");
}
#else
